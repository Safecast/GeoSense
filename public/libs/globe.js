...
DAT.Globe = function(container, colorFn) {
	...
	var distance = distanceTarget = radius * 3;
	var padding = 40;
	var PI_HALF = Math.PI / 2;

	var barHeight = radius / 4;
	var barWidth = barHeight / 20;
	...
	function init() {
		...
		camera = new THREE.PerspectiveCamera(CAMERA_FOV, aspectRatio, 1, 1e7);
	...
	addData = function(data, opts) {
		...
		var colorFunction = opts.colorFn || colorFn; 

		if (opts.format === 'magnitude') {
			step = 3;
			colorFnWrapper = function(data, i) { return colorFunction(data[i+2]); }
		} else if (opts.format === 'legend') {
			step = 4;
			colorFnWrapper = function(data, i) { return colorFunction(data[i+3]); }
		} else {
