                ...
                function onTransformData(saveModel, transformed)
                {
                    ...
                    if (doSave) {
                        if (parser.readStream) {
                            ...
                            // Update: Node 0.10+ creates streams in non-flowing mode by default.
                            // They are not paused, but read() must be explicitly called.
                            // https://stackoverflow.com/questions/18306954/cannot-switch-to-old-mode-now-node-js-apn-module-error-in-tls-connect-functi
                            if (!parser.readStream.read) {
                                // However the streams return by the `request` library do not have a `read` method
                                // and need to be treated like the old flowing streams.
