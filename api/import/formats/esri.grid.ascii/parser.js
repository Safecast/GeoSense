...
// Experimental ESRI ASCII grid parser by Samuel Luescher <sam@luescher.org> 
// based on
...
module.exports = function(){
            ...
            switch (c) {
                ...
                case csv.readOptions.delimiter:
                    ...
                    if( state.quoted ) {
                        ...
                        state.line.push(state.field);
                        state.field = '';
                        // current row of cells is terminated because reaching end of 
                        if (csv.readOptions.forceLineBreakAfter && state.line.length >= csv.readOptions.forceLineBreakAfter) {
                            flush();
                        }
                    }
                    break;
               case '\n':
                ...
                case '\r':
                    ...
                    // only flush if field is not empty (since row cells may have been terminated before line break)
                    if (state.field != '') {
                        state.line.push(state.field);
                        state.field = '';
                        flush();
    ...
    function flush(){
        for (var i = 0; i < state.line.length; i++) {
            var number = state.line[i] != '' ? Number(state.line[i]) : state.headerValues.NODATA_value;
            state.line[i] = !isNaN(number) ? number : state.line[i];
        }
        if (state.isHeader) {
            if (csv.readOptions.header.indexOf(state.line[0]) != -1) {
                state.headerValues[state.line.shift()] = state.line.pop();
                state.line = [];
                state.lastC = '';
                return;
            } else {
        ...
        if(!preserve){
            try {
                for (var i = 0; i < line.length; i++) {
                    if (line[i] != state.headerValues.NODATA_value) {
                        //console.log(line[i]);
                        ...
                        csv.emit('data', data, state.count);
                    } else {
                        //console.log('skip', state.colIndex, state.rowIndex);
                    }
                    state.colIndex++;
                    if (state.colIndex >= state.headerValues.ncols) {
                        state.colIndex = 0;
                        state.rowIndex++;
                    }
                    /*
